...
import { CellDepOptionsLike, ScriptOptionsLike } from "./types.js";
...
export function getNetworkConfig(client: ccc.Client) {
  ...
  return isMainnet
    ? CKB_TIME_TYPE_CONFIG.mainnet
    : CKB_TIME_TYPE_CONFIG.testnet;
}

export function resolveScript(client: ccc.Client, options?: ScriptOptionsLike) {
...
export function resolveCellDep(
  client: ccc.Client,
  options?: CellDepOptionsLike,
) {
  const config = getNetworkConfig(client);
  return {
